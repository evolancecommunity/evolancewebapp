...
DB_NAME = "Journal_space_db"
music_collection_name = "music preferences"
journal_collection_name ="journal_entries"
...
async def get_music_preferences(user_id: str):
 ...
 # Endpoints to save a journey entry

@api_router.post("/journey/save", response_model=JourneyEntry, summary="Save a journey entry")
async def save_journey_entry(entry: JourneyEntry):
    """
    Saves a journey entry.
    """
    if not client:
        raise HTTPException(status_code=503, detail="Journey service is not available")
    if not journal_collection:
        raise HTTPException(status_code=503, detail="Journal collection is not initialized")
    detected_mood = None
    if entry.content and sentiment_pipeline:
        try:
            sentiment_result = sentiment_pipeline(entry.content[0]) 
            sentiment_label = sentiment_result['label']
            detected_mood = map_sentiment_to_mood(sentiment_label)
            entry.mood = detected_mood
            print(f"Journal Entry Text: '{entry.content[:50]}'Detected sentimental: {sentiment_label},Mapped mood: {detected_mood}")
        except Exception as e:
            print(f"Error analyzing sentiment for journey entry: {e}")
            entry.mood = "Error Analyzing"
    try:
        # Insert the journey entry into the new collection
        result = await journal_collection.insert_one(entry.model_dump(by_alias=True))
        return {"message": "Journal entry saved successfully", "id": str(result.inserted_id)}
    except Exception as e:
        raise HTTPException(status_code = 500, detail= f"Failed to save journey entry: {e}" )

@api_router.get("/journal/recent/{user_id}")   
async def get_recent_journal_entries(user_id: str):
    """Retrieves the 5 most recent journal entries"""     
    if not client:
        raise HTTPException(status_code=503, detail ="Database connection not established")
    if not journal_collection:
        raise HTTPException(status_code=503, detail = "Journal collection not initialized")

    try:
        entries=[]

        docs = journal_collection.find({"userId": user_id}).sort("timestamp", -1).limit(5)
